// constants
const BUBBLE_FRAMES = Array.from({ length: 4 }, (_, i) =>
  `Assets/fish-photobooth/homepage/animated-bubbles-home/${i + 1}.png`
);

const PHOTOBOOTH_FRAMES = Array.from({ length: 27}, (_, i) =>
...
function animatePhotobooth(timestamp) {
  ...
  if (timestamp - lastFrameTime >= PHOTOBOOTH_FRAME_INTERVAL) {
    ...
    if (frame.complete) {
      ...
      // Move to the next frame (looping)
